...
// 🔔 마감 표시 이벤트는 할일을 통해 다루는 것이라 후보에서 뺀다 (taskCommit.js가
// deadline 필드 변경 시 이 이벤트를 알아서 만들고/지운다).
//
...
export async function getExistingItems(userId, refreshToken) {
  ...
  for (const e of events) {
    if ((e.summary ?? "").startsWith(DEADLINE_PREFIX)) continue;
    const dateStr = e.start.date ?? e.start.dateTime?.slice(0, 10) ?? null;
    items.push({ kind: "event", id: e.id, title: e.summary || "(제목 없음)", dateStr, tracked: false });
  }
...
export function formatExistingItemsForPrompt(items) {
  ...
  return items
    .map((item, i) => {
      const kindLabel = item.kind === "event" ? "일정" : "할일";
