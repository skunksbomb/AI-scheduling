...
export async function syncWithGoogle() {
  ...
  if (tasks.length === 0) {
    await importNewGoogleTasks(tasks);
    return getTasks();
  }
  ...
  const [remoteTasks, remoteEvents] = await Promise.all([
    needsTaskList ? listAllTasks() : Promise.resolve([]),
    deadlineDates.length > 0 ? listDeadlineEventsCovering(deadlineDates) : Promise.resolve([]),
  ]);
...
async function listDeadlineEventsCovering(deadlineDates) {
