      ...
      if (existing.kind === "event") {
        const label = item.hasTime
          ? (() => {
              ...
              return parsed ? `${formatKoreanDate(parsed.dateStr)} ${String(Math.floor(parsed.minutes / 60)).padStart(2, "0")}:${String(parsed.minutes % 60).padStart(2, "0")}` : item.startTime;
            })()
          : `${formatKoreanDate(String(item.startTime).slice(0, 10))} 하루종일`;
        drafts.push({
          op: "edit",
          kind: "event",
          existingId: existing.id,
