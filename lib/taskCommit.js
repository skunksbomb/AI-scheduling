...
export async function commitNewTaskPlacement(refreshToken, item, rawText) {
  ...
  try {
    ...
    base.scheduleError = reauthMessageFor(err) ?? err.message;
  }
...
export async function commitEventItem(refreshToken, item) {
  ...
  try {
    ...
    const reason = reauthMessageFor(err) ?? "";
...
export async function commitDeleteItem(userId, refreshToken, item) {
  try {
    ...
    const reason = reauthMessageFor(err) ?? err.message;
...
export async function commitEditItem(userId, refreshToken, item) {
  try {
    ...
    // 별개 개념이라 안 건드림 — 단, 마감을 실행일보다 앞으로 당기면 실행일도
    // 같이 당겨야 "마감 지난 뒤에 하기로 되어있는" 상태가 안 생긴다). 추적
    // 안 되는 할일은 구글 Task 자체엔 "마감"이라는 별도 필드가 없고 due
    // 하나뿐이라, 그 due를 직접 바꾼다 — lib/existingItems.js가 이 둘을
    // 구분해서 AI에게 보여주는 것과 짝을 이룬다.
    const tasks = await getTasks(userId);
    const row = tasks.find((t) => t.googleTaskId === item.existingId);

    if (row) {
      const patch = { title: item.title };
      let finalScheduledDate = row.scheduledDate;
      let finalStartMinutes = row.suggestedStartMinutes;
      let finalEndMinutes = row.suggestedEndMinutes;
      ...
      if (item.deadline !== row.deadline) {
        ...
        // 새 마감이 지금 실행일보다 이르면 실행일도 마감을 넘지 않게 당긴다.
        // 이전에 잡아둔 시간대는 당겨진 날짜엔 유효한지 알 수 없으니 같이 지운다.
        if (item.deadline && row.scheduledDate && item.deadline < row.scheduledDate) {
          finalScheduledDate = item.deadline;
          finalStartMinutes = null;
          finalEndMinutes = null;
          patch.scheduledDate = finalScheduledDate;
          patch.suggestedStartMinutes = null;
          patch.suggestedEndMinutes = null;
        }
      }

      await patchTask(refreshToken, item.existingId, {
        title: formatGoogleTaskTitle({
          title: item.title,
          hasSuggestedTime: finalStartMinutes != null && finalEndMinutes != null,
          suggestedStartMinutes: finalStartMinutes,
          suggestedEndMinutes: finalEndMinutes,
        }),
        dueDateStr: finalScheduledDate,
      });
