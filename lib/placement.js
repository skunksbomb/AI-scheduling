...
import { buildDateTable, createChatCompletion } from "@/lib/ai";
...
// 구글 캘린더 dateTime은 "+09:00" 오프셋이 이미 붙어있는 KST 문자열이라,
// Date 객체를 거치지 않고 문자열에서 바로 날짜/시각을 읽는다 — Date를 거치면
// 서버가 UTC로 도는 환경(Vercel)에서 시/분이 9시간 밀린다.
function dateTimeToDateAndMinutes(iso) {
  return parseStartTime(iso);
}
...
export async function suggestPlacements({ items, busyEvents, busyTasks, windowTo, userContext = [], latestFeedback = null }) {
  ...
  const dayCount = Math.max(7, Math.min(90, daysBetween(today, windowTo) + 3));
  ...
  const itemLines = items
    .map((item, index) => {
      ...
      const timeHint = item.roughTimeOfDay ? ` / 원하는 시간대: ${item.roughTimeOfDay} (이 시간대 안에서 고를 것)` : "";
...
${
  latestFeedback
    ? `\n사용자가 이 배치에 대해 지금까지 시간순으로 준 피드백 — 전부 계속 지키되, 가장 마지막
피드백을 최우선으로 반영할 것:\n${latestFeedback}\n`
    : ""
}
배치할 할 일 목록:
...
- "원하는 시간대"가 적힌 항목은 반드시 hasSuggestedTime: true로 하고, 그 시간대 범위 안에서 겹치지
  않는 시각을 골라야 합니다(예: "아침"이면 대략 06:00~10:00 사이). 사용자가 직접 요청한 시간대를
  무시하고 다른 시간대에 배치하면 안 됩니다.
...
설명 없이 다음 형식의 JSON 객체 하나만 출력하세요:
{
  ...
  const completion = await createChatCompletion(client, requestBody);
...
export async function buildTaskDraft({ userId, refreshToken, taskItems, userContext, latestFeedback = null }) {
  const today = todayStr();
  ...
  try {
    const [busyEvents, existingTasks] = await Promise.all([
      listEvents(refreshToken, `${today}T00:00:00+09:00`, `${addDays(windowTo, 1)}T00:00:00+09:00`),
      getTasks(userId),
    ]);
    ...
    const dayCounts = buildDayCounts(await getTasks(userId));
...
const RECURRENCE_FREQ_LABEL = { DAILY: "매일", WEEKLY: "매주", MONTHLY: "매달" };

function recurrenceSuffix(item) {
  if (!item.recurring || !RECURRENCE_FREQ_LABEL[item.recurrenceFreq]) return "";
  const until = item.recurrenceUntil ? ` (${formatKoreanDate(item.recurrenceUntil)}까지)` : "";
  return ` — ${RECURRENCE_FREQ_LABEL[item.recurrenceFreq]} 반복${until}`;
}

// 일정(event) 중 정확한 시각 없이 "아침에"/"오후에"처럼 대략적인 시간대만
// 있는 것들에게, 그날 실제 캘린더를 보고 구체적인 시각을 골라준다. 이 앱의
// 정체성이 "언제 할지 정하는 에너지를 아예 없애는 것"이라 시각도 AI가 정해야
// 하고, 시간 없는 하루종일 일정으로 방치하면 안 된다. AI가 확신 못 하거나
// 겹치는 시간만 고르면, 무리하지 않고 원래대로 하루종일로 남긴다.
export async function resolveEventTimeHints(refreshToken, eventItems) {
  const needsTime = eventItems.filter((item) => !item.hasTime && item.roughTimeOfDay);
  if (needsTime.length === 0) return eventItems;

  const dateStrs = needsTime
    .map((item) => String(item.startTime).match(/^(\d{4}-\d{2}-\d{2})/)?.[1])
    .filter(Boolean);
  if (dateStrs.length === 0) return eventItems;

  const from = dateStrs.reduce((a, b) => (a < b ? a : b));
  const to = dateStrs.reduce((a, b) => (a > b ? a : b));

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return eventItems;

  let busyEvents;
  try {
    busyEvents = await listEvents(refreshToken, `${from}T00:00:00+09:00`, `${addDays(to, 1)}T00:00:00+09:00`);
  } catch {
    return eventItems;
  }

  const busyBlock = (() => {
    const lines = buildBusyEventLines(busyEvents);
    return lines.length > 0 ? lines.join("\n") : "(해당 기간 등록된 일정 없음)";
  })();

  const itemLines = needsTime
    .map((item, index) => {
      const dateStr = String(item.startTime).match(/^(\d{4}-\d{2}-\d{2})/)[1];
      return `[${index}] "${item.title}" / 날짜: ${dateStr} / 원하는 시간대: ${item.roughTimeOfDay} / 예상 소요시간 ${
        item.estimatedMinutes ?? 60
      }분`;
    })
    .join("\n");

  const systemPrompt = `사용자가 적은 일정(약속) 중 정확한 시각 없이 "아침에"/"오후에" 같은 대략적인
시간대만 있는 것들이 있습니다. 실제 캘린더를 참고해서 각 항목에 구체적인 시작/종료 시각(자정 기준
분 단위)을 정해주세요.

기존에 이미 잡혀있는 일정 (겹치지 않게 배치할 것):
${busyBlock}
...
규칙:
- "원하는 시간대" 표현이 뜻하는 상식적인 시간 범위 안에서 고르세요(예: 아침이면 대략 07:00~11:00,
  오전이면 09:00~12:00, 점심이면 12:00~13:00, 오후면 13:00~18:00, 저녁이면 18:00~21:00). 그 범위
  안에서도 사람들이 보통 약속을 잡는 자연스러운 시각(정각/30분 단위, 너무 이른 새벽 아님)을 고르세요.
- 위 "기존에 이미 잡혀있는 일정"과 겹치지 않게 하세요.
- 시각을 정하지 않고 남기는 것(confident: false)은 정말 최후의 수단입니다 — 요청한 시간대 전체가
  이미 다른 일정으로 빈틈없이 차 있어서 물리적으로 넣을 자리가 없을 때만 그렇게 하세요. 시간대가
  조금 붐비거나 애매한 정도로는 반드시 시각을 정하세요. 사용자는 "언제 할지"를 직접 고르고 싶지
  않아서 이 앱을 쓰는 것이므로, 시각 없이 돌려주면 그 고민이 사용자에게 그대로 넘어갑니다.

설명 없이 다음 형식의 JSON 객체 하나만 출력하세요:
{
  "resolutions": [
    { "index": 0, "confident": true 또는 false, "startMinutes": 정수 또는 null, "endMinutes": 정수 또는 null }
  ]
}`;
  ...
  try {
    completion = await createChatCompletion(client, {
      ...
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: "위 정보를 바탕으로 시각을 정해주세요." },
      ],
    });
  } catch {
    return eventItems;
  ...
  if (!Array.isArray(parsed.resolutions)) return eventItems;

  const resultByNeedsTimeIndex = new Map(parsed.resolutions.map((r) => [r.index, r]));

  return eventItems.map((item) => {
    const needsTimeIndex = needsTime.indexOf(item);
    if (needsTimeIndex === -1) return item;

    const res = resultByNeedsTimeIndex.get(needsTimeIndex);
    const validRange =
      res?.confident &&
      Number.isInteger(res.startMinutes) &&
      Number.isInteger(res.endMinutes) &&
      res.startMinutes >= 0 &&
      res.endMinutes <= 1439 &&
      res.endMinutes > res.startMinutes;
    if (!validRange) return item;

    const dateStr = String(item.startTime).match(/^(\d{4}-\d{2}-\d{2})/)[1];
    if (findTimeConflict(dateStr, res.startMinutes, res.endMinutes, busyEvents)) return item;

    return {
      ...item,
      hasTime: true,
      startTime: toLocalDateTime(dateStr, res.startMinutes),
      estimatedMinutes: res.endMinutes - res.startMinutes,
    };
  });
}
...
export function buildEventDraftItems(eventItems) {
  return eventItems.map((item) => {
    ...
    const suffix = recurrenceSuffix(item);
    ...
    if (!item.hasTime) {
      ...
      return {
        ...item,
        displayLine: dateStr
          ? `📅 ${formatKoreanDate(dateStr)} 하루종일 "${title}"${suffix}`
    ...
    return {
      ...
      displayLine: `📅 ${formatKoreanDate(parsedStart.dateStr)} ${formatMinutesAsTime(
        parsedStart.minutes
      )}~${formatMinutesAsTime(endMinutes)} "${title}"${suffix}`,
    };
  });
}
