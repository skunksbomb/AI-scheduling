...
- "task": 실행해서 끝내야 하는 작업. deadline에 관련 날짜를 채우고 startTime은 null로 둔다. 두 가지로
  ...
  - 정확히 그 날 하겠다고 못박음(exact: true): 예: "내일 아침에 수학숙제 하기", "오늘 저녁에 청소하기".
    ...
    그 날짜를 채우고, 다른 날로 옮기면 안 된다. "다음주 서울 갔을 때 안경집 가기"처럼 개인 컨텍스트의
    반복 패턴(장소/계기)에 엮인 경우도 이쪽이다 — "언제까지 해도 됨"이 아니라 "그 계기가 되는 특정
    날짜 하나"를 가리키는 것이므로 마감이 아니다. 아래 날짜 해석 규칙 참고.
...
- 사용자 개인 컨텍스트에 반복되는 요일 패턴(예: "매주 금요일마다 서울감", "매주 일요일마다 대전으로
  ...
  "다음주 서울 갔을 때", "서울 올라가서"), 이건 "언제까지 해도 되는 일(마감)"이 아니라 "그 계기에
  하는 일"이므로 deadline이 아니라 exact: true로 처리한다.
  - 여러 날짜를 놓고 고민하거나 기간을 계산하지 마세요. 텍스트가 가리키는 상황과 가장 직접적으로
    일치하는 컨텍스트 문장을 하나 고르고, 그 문장에 적힌 요일을 그대로 표에서 찾아 쓰세요.
  ...
  - exact: true이므로 deadline에는 그렇게 찾은 하나의 날짜만 채운다.
