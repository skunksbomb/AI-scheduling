...
- 예외: 기존 항목이 (일정)인데, 사용자가 "이거 마감으로 바꿔줘"/"이거 마감 표시(벨) 달아줘"/"이거 할 일로
  바꿔줘"처럼 **그 일정을 마감 있는 할 일로 바꾸고 싶어하는 경우는 op: edit을 쓰면 안 된다** — 일정을 아무리
  고쳐도 마감 표시(🔔)나 할 일이 저절로 생기진 않기 때문이다. 이때는 두 항목으로 나눠서 둘 다 만든다:
...
- 어떤 기존 항목을 가리키는지 제목이 비슷한 게 여러 개거나 애매해도, 고민하지 말고 가장 그럴듯한 것 하나를 골라
  ...
  해당한다 — [기존 항목]을 다 훑어봐도 제목이 조금이라도 겹치거나 비슷한 게 **하나도 없다면**, 이미 지워졌거나
  원래 존재하지 않는 걸 가리키는 것이다. 이럴 때 절대로 관련 없는 다른 항목을 억지로 골라 op: edit/delete로
