...
// o1/o3 같은 reasoning 계열, 그리고 이후 나온 일부 모델(gpt-5.6 계열 등)은
// temperature 커스텀 값 자체를 거부하고 기본값(1)만 허용한다. 어떤 모델이
// 이런 제약을 갖는지 이름으로 미리 목록을 관리하면 새 모델이 나올 때마다
// 코드를 고쳐야 하니, 대신 실패하면 temperature 없이 한 번 재시도한다.
export async function createChatCompletion(client, requestBody) {
  try {
    return await client.chat.completions.create(requestBody);
  } catch (err) {
    if (err?.param === "temperature" || /temperature/i.test(err?.message ?? "")) {
      const { temperature, ...rest } = requestBody;
...
// AI는 표에서 찾기만 하게 한다.
//
// 기본 90일: 예전엔 21일(3주)이었는데, "4주 뒤 화요일"처럼 표 밖의 날짜를
// 말하면 AI가 표 지시를 무시하고 스스로 계산해버렸다 — 표를 만든 이유(계산
// 실수 방지)가 그 구간에선 통째로 사라지는 셈. 90일이면 일상적인 상대 날짜
// 표현("N주 뒤", "다다다음주", "다음달 X요일")이 거의 다 표 안에 들어온다.
export function buildDateTable(days = 90) {
  const rows = [];
...

{
  ...
  "tasks": [
    {
      "op": "add(새 항목 추가, 기본값) 또는 edit(아래 [기존 항목] 중 하나를 수정) 또는 delete(아래 [기존 항목] 중 하나를 삭제)",
      "existingIndex": "op이 edit 또는 delete일 때만: 아래 [기존 항목] 목록에서의 번호(정수). op이 add면 null",
      "type": "event" 또는 "task",
      ...
      "recurring": "type이 event일 때만 사용, 그 외엔 false. '매주/매일/매달 ~한다'처럼 반복을 요구하면 true",
      "recurrenceFreq": "recurring이 true일 때만: 'DAILY'(매일) 또는 'WEEKLY'(매주) 또는 'MONTHLY'(매달). 그 외엔 null",
      "recurrenceUntil": "recurring이 true이고 '~까지'처럼 종료 시점이 언급됐을 때만 'YYYY-MM-DD'. 종료 시점 언급이 없으면(무기한 반복) null",
      "urgent": true 또는 false,
      "important": true 또는 false,
      "estimatedMinutes": 예상 소요 시간(분, 정수), 모르면 60,
      "roughTimeOfDay": "텍스트에 '아침에'/'오전에'/'점심때'/'오후에'/'저녁에'/'밤에'처럼 대략적인 시간대
        표현이 있으면 그 표현을 그대로 담는다(예: '아침'). 이미 정확한 시각('3시')이 있거나 시간대
        표현이 전혀 없으면 null. type/hasTime/exact와 무관하게 이 표현이 있으면 항상 채운다 — 이 필드가
        있어야 몇 시가 좋을지 판단하는 다음 단계에서 사용자가 원한 시간대를 반영할 수 있다."
    }
  ]
}
...
- "event": 다른 사람과의 약속/회의/행사처럼 특정 날짜에 벌어지는 일이거나, **어딘가에 가는/방문하는
  ...
  절대 금지 — "아침에"/"오후에"처럼 대략적인 시간대만 있을 때 정확한 시각을 여기서 지어내지 말고,
  대신 roughTimeOfDay에 그 표현을 담아둔다(구체적인 시각은 실제 캘린더를 보는 다음 단계에서 정해짐).
...
반복(recurring) 판단 기준 — event에만 해당:
- "매주 화요일마다 헬스", "매일 아침 스트레칭", "매달 1일 정산"처럼 **dump에 지금 적은 문장 자체가
  반복을 요구할 때만** recurring: true로 한다. "내 상황"(개인 컨텍스트)에 적혀있는 반복 패턴은 날짜
  해석 참고용일 뿐 이걸로 반복 일정을 만들면 안 된다 — 오직 이번에 사용자가 직접 쓴 문장만 본다.
  task(할일)는 구글 할일 자체에 반복 기능이 없어 지원하지 않으니, 반복 표현이 있어도 recurring은
  건드리지 말고 false로 둔다.
- startTime에는 반복이 시작되는 첫 날짜(요일 표에서 찾은 날)를 그대로 채운다 — 요일 코드 같은 건
  따로 계산할 필요 없다(코드가 시작 날짜로부터 알아서 계산함).
- "이번 학기까지"/"12월까지"/"올해까지"처럼 끝나는 시점이 언급되면 recurrenceUntil에 그 날짜를,
  언급이 없으면 null(무기한 반복)로 둔다.
...
[기존 항목] (최근~다가오는 일정/할일 목록. 사용자가 이 중 하나를 수정하거나 삭제하려는 것인지 판단할 때 씀):
{{EXISTING_ITEMS}}

수정(edit)/삭제(delete) 판단 기준:
- 사용자가 "취소해줘", "지워줘", "없애줘"처럼 기존 항목을 없애려 하면 op: delete. existingIndex에 [기존 항목]에서 가장
  가리키는 게 명확한 번호를 넣고, type/title은 그 기존 항목 것을 그대로 복사한다 (나머지 필드는 무시되니 신경 쓰지 않아도 됨).
- 사용자가 "~로 바꿔줘", "~로 미뤄줘/당겨줘", "제목을 ~로 고쳐줘"처럼 기존 항목의 날짜/시간/마감/제목을 바꾸려 하면
  op: edit. existingIndex를 채우고, title/deadline/exact/hasTime/startTime 등은 **바뀐 뒤의 최종 상태**를 전부
  채운다 — 바뀌지 않는 필드도 [기존 항목]에 적힌 값을 그대로 옮겨써야 한다 (비워두면 안 됨). 날짜 표현은 위 날짜
  해석 규칙과 표를 그대로 적용해서 해석한다.
- 예외: 기존 항목이 (일정)인데, 문장에서 그 항목에 "마감"이라는 개념이 등장하면(예: "이거 마감으로
  바꿔줘", "마감 표시 달아줘"처럼 명시적으로 바꿔달라고 하는 경우는 물론, "오늘 어드리 과제 마감임",
  "그거 마감이야"처럼 바꿔달라는 말 없이 그냥 마감일을 알려주기만 하는 경우도 전부 포함) **op: edit을
  쓰면 안 된다** — 일정(event)에는 애초에 "마감"이라는 필드 자체가 없어서, 마감 언급이 나온 순간
  이 항목은 이미 일정이 아니라 할 일(마감형)이어야 하는 것이다. 일정을 아무리 고쳐도 마감 표시(🔔)나
  할 일이 저절로 생기진 않는다. 이때는 두 항목으로 나눠서 둘 다 만든다:
  (1) 그 기존 일정은 op: delete로 지우고 (existingIndex/title은 delete 규칙대로 기존 항목 것을 그대로 복사),
  (2) 완전히 새로운 op: add, type: task 항목을 하나 더 만들어 제목과 마감(deadline)에 원하는 날짜를 채운다
  (exact는 false로 두어, 마감일 안에서 정확히 언제 할지는 AI가 새로 판단하게 한다). 같은 일을 가리키는
  두 항목(예: 방금 지운 일정 제목과 겹치는 완전히 별개의 할 일)을 동시에 add로 또 만들지 않는다 — 위
  (1)(2) 두 항목이 이 요청 전체에 대한 답이다.
- 위 예외는 "마감이 **새로 생기는**" 경우에만 해당한다. 아래 두 경우는 여기 해당하지 않으며,
  마감(deadline)을 지어내면 안 된다:
  - 마감을 **없애라**는 요청: "마감 빼줘", "마감 일정 지워줘", "마감 표시 없애줘", "이건 마감 아니야"처럼.
    문장에 "마감"이라는 단어가 들어있다는 이유만으로 마감을 새로 만들면 절대 안 된다 — 오히려 정반대
    요청이다. 이때는 그 항목의 deadline을 반드시 null로 둔다 (할 일 자체는 그대로 두고 마감 날짜만 뺀다).
    기존 (할일) 항목을 가리키는 요청이면 op: edit + existingIndex를 채우고 deadline: null로 한다.
  - 마감 언급 없이 **종류만** 바꾸라는 요청: "이 일정 할일로 바꿔줘"처럼. 이때 원래 일정에 적힌 날짜는
    기한이 아니라 "그 날 하겠다"는 실행 날짜이므로, (1) 기존 일정 delete + (2) op: add, type: task를
    만들되 deadline에 그 날짜를 넣고 **exact: true**로 한다 (exact: true면 마감 표시 🔔 없이 그 날 할
    일로만 배치된다). 사용자가 "마감"이라고 직접 말했을 때만 exact: false로 둔다.
- 기존 항목을 가리키는 방식은 제목만이 아니다. "금요일 일정 삭제해", "내일 약속 취소해", "9월 12일 거 미뤄줘"처럼
  **제목 없이 날짜/요일만으로** 가리키는 경우도 흔하다. 이때는 제목 유사도를 따지지 말고, 날짜 표를 써서 그
  표현이 가리키는 날짜를 먼저 정한 뒤 [기존 항목] 중 그 날짜에 있는 항목을 찾는다. 그 날짜에 항목이 하나뿐이면
  그게 답이고, 여러 개면 문장의 다른 단서(일정/할일 여부, 시간대, 대상 등)로 가장 그럴듯한 하나를 고른다.
  "그 날짜에 아무것도 없을 때"만 존재하지 않는 항목으로 본다.
- 어떤 기존 항목을 가리키는지 제목이 비슷한 게 여러 개거나 애매해도, 고민하지 말고 가장 그럴듯한 것 하나를 골라
  진행한다 (사용자가 확정 전 미리보기에서 확인하고 취소할 수 있으니 괜찮다). 단, 이건 후보가 최소 하나는 있을 때만
  해당한다 — [기존 항목]을 다 훑어봐도 제목이 겹치는 것도, 가리킨 날짜에 있는 것도 **하나도 없다면**, 이미
  지워졌거나 원래 존재하지 않는 걸 가리키는 것이다. 이럴 때 절대로 관련 없는 다른 항목을 억지로 골라 op: edit/delete로
  처리하면 안 된다(엉뚱한 진짜 일정이 수정/삭제될 위험이 있다) — 이 항목은 통째로 무시하고 tasks 배열에 넣지
  않는다. 이 경우 uncertain: true로 하고 reason에 "이미 지워졌거나 존재하지 않는 항목을 가리키는 것 같습니다"
  같은 이유를 적는다.
- 위 "이미 지워졌거나 존재하지 않는 항목을 가리키는 것 같습니다"는 사용자가 취소/삭제/변경처럼 기존
  항목을 다루려는 의도가 분명한데 후보를 못 찾았을 때만 쓰는 문구다. "이거 좀 처리해야되는데"처럼
  기존 항목을 고치려는 게 아니라 그냥 새 할 일을 막연하게 적었는데 "이거"/"그거" 같은 지시어라 뭘
  가리키는지 알 수 없는 경우는 다른 상황이다 — 이때는 (위 "tasks를 빈 배열로 반환하는 것에 대한
  경고"에 따라) tasks를 비우지 말고 op: add로 지시어를 title에 그대로 살려("이거 처리하기") 항목을
  채우고, uncertain: true로 하되 reason엔 "무엇을 가리키는지 구체적이지 않음"처럼 실제 이유를 적는다
  (존재하지 않는 기존 항목을 가리킨다는 문구를 여기 쓰면 안 된다 — 애초에 기존 항목을 가리키려던 게
  아니었기 때문이다).
- 위 경우들이 아니면(완전히 새로운 내용이 명확하면) op: add — 지금까지의 모든 규칙을 그대로 적용한다.
...
export async function parseDumpText(text, userContext = [], existingItemsText = "(현재 등록된 일정/할일 없음)") {
  ...
  const systemPrompt = SYSTEM_PROMPT.replace("{{DATE_TABLE}}", buildDateTable())
    .replace("{{USER_CONTEXT}}", formatUserContext(userContext))
    .replace("{{EXISTING_ITEMS}}", existingItemsText);
