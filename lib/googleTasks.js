...
// maxResults/pageToken 없이 한 페이지만 읽으면 기본 페이지 크기를 넘는 계정에서
// 최근 할 일이 조용히 빠지는 실제 버그가 있었다 — listAllTasks와 동일하게
// 끝까지 페이지를 순회한다.
...
// title/dueDateStr 중 넘긴 것만 바뀐다. completeTask/reopenTask와 달리
// 완료 상태(status)는 안 건드린다.
export async function patchTask(refreshToken, taskId, { title, dueDateStr }) {
  const tasks = getClient(refreshToken);
  const requestBody = {};
  if (title != null) requestBody.title = title;
  if (dueDateStr) requestBody.due = `${dueDateStr}T00:00:00.000Z`;
  const res = await tasks.tasks.patch({ tasklist: "@default", task: taskId, requestBody });
  return res.data;
}
