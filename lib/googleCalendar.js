...
export async function createEvent(refreshToken, { title, startISO, endISO, description }) {
  const calendar = getClient(refreshToken);
...
export async function createAllDayEvent(refreshToken, { title, dateStr, nextDateStr }) {
  const calendar = getClient(refreshToken);
  const res = await calendar.events.insert({
    calendarId: "primary",
    requestBody: {
